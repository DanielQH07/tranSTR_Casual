...
def accuracy_metric_cvid(result_file, gt_file=None):
    """
    ...
    for qid in preds.keys():
        ...
        qtype_str = QTYPE_NUM_TO_STR[qtype_num]
    ...
    for qtype, qids in qns_group.items():
        ...
        for qid in qids:
            ...
            if answer == pred: 
                acc += 1
                # Check combined accuracy for predictive and counterfactual
                if qtype == 'p':
                    # Get corresponding reason prediction
                    vid = qid.rsplit('_', 1)[0]
                    reason_qid = vid + '_3'  # predictive reason
                    if reason_qid in preds:
                        if preds[reason_qid]['answer'] == preds[reason_qid]['prediction']:
                            acc_par += 1
                elif qtype == 'c':
                    # Get corresponding reason prediction
                    vid = qid.rsplit('_', 1)[0]
    ...
    for qtype, acc in group_acc.items():
        if group_cnt[qtype] > 0:
            results_str.append('{:.2f}'.format(acc*100.0/group_cnt[qtype]))
        else:
            results_str.append('N/A')
    logger.debug("     ".join(results_str))
    if all_cnt > 0:
        logger.debug('Acc: {:.2f}'.format(all_acc*100.0/all_cnt))
    else:
        logger.debug('Acc: N/A')
    
    return all_acc*100.0/all_cnt if all_cnt > 0 else 0 
